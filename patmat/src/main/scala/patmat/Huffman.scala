...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.length == 1) trees
      ...
      val newTreeList = trees.tail.tail
      insertTreeInList(newTreeList, newTree)
    }
  }

  def insertTreeInList(trees: List[CodeTree], newTree: CodeTree): List[CodeTree] = {
    if (trees.isEmpty) List(newTree)
    else if (weight(newTree) < weight(trees(0))) newTree :: trees
    else trees.head :: insertTreeInList(trees.tail, newTree)
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    if (bits.isEmpty) Nil
      ...
      val pair = decodeLetter(tree, bits)
      pair._1 :: decode(tree, pair._2)
    }
  }

  def decodeLetter(tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
    tree match {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    if (text.isEmpty) Nil
    else encodeLetter(tree, text.head) ::: encode(tree)(text.tail)
  }

  def encodeLetter(tree: CodeTree, char: Char): List[Bit] = {
    tree match {
      case Leaf(x, _) => Nil
      case Fork(left, right, charList, _) => {
        if (chars(left).contains(char)) 0 :: encodeLetter(left, char)
        else 1 :: encodeLetter(right, char)
      }
    }
  }
  ...
  def prepend(x: Bit, table: CodeTable): CodeTable = {
    if (table.isEmpty) Nil
    else (table.head._1, x :: table.head._2) :: prepend(x, table.tail)
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def iterate(text: List[Char]): List[Bit] = {
      if (text.isEmpty) Nil
      else codeBits(table)(text.head) ::: iterate(text.tail)
    }
    iterate(text)
  }
}
